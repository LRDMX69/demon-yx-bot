 ...
 * does not need thousands of hand-maintained plugin files. Existing command
 * names and aliases remain stable; extended namespaces add more than 10,000
 * new discoverable capabilities without shadowing the original surface.
 */
...
const extendedObjects = [
  'account', 'action', 'alert', 'answer', 'archive', 'asset', 'attachment', 'audit', 'backup', 'badge', 'batch', 'board', 'bookmark', 'broadcast', 'calendar', 'channel', 'checklist', 'citation', 'client', 'cluster', 'collection', 'command', 'contact', 'context', 'cookie', 'dashboard', 'dataset', 'device', 'digest', 'document', 'domain', 'draft', 'event', 'example', 'export', 'feed', 'file', 'filter', 'folder', 'form', 'group', 'guide', 'history', 'identity', 'inbox', 'index', 'insight', 'invite', 'item', 'job', 'knowledge', 'label', 'link', 'list', 'log', 'message', 'metric', 'model', 'note', 'notification', 'object', 'option', 'package', 'page', 'permission', 'pipeline', 'plan', 'policy', 'profile', 'prompt', 'queue', 'record', 'report', 'request', 'resource', 'response', 'result', 'role', 'rule', 'schedule', 'schema', 'search', 'secret', 'session', 'setting', 'source', 'status', 'stream', 'summary', 'tag', 'task', 'template', 'token', 'topic', 'trace', 'user', 'variable', 'version', 'view', 'webhook', 'workflow', 'workspace',
]
const extendedProfiles = [
  'automation', 'knowledge', 'communication', 'workflow', 'content', 'monitoring', 'integration', 'data', 'community', 'research',
].map((category) => [category, ['inspect', 'create', 'update', 'find', 'plan', 'run', 'review', 'export', 'monitor', 'organize'], extendedObjects])
...
function addProfileCommands([category, verbs, objects], legacyNames = false) {
  for (const verb of verbs) {
    for (const object of objects) {
      const name = legacyNames ? `dx-${slug(verb)}-${slug(object)}` : `dx-${slug(category)}-${slug(verb)}-${slug(object)}`
      const commandAliases = legacyNames
        ? [`${slug(category)}-${slug(verb)}-${slug(object)}`, `${slug(verb)}-${slug(object)}`]
        : [`${slug(category)}-${slug(verb)}-${slug(object)}`]
      addCommand({
...
for (const profile of profiles) addProfileCommands(profile, true)
for (const profile of extendedProfiles) addProfileCommands(profile, false)
...
function searchCommands(query, limit = 12) {
  ...
  if (!q) return commands.slice(0, boundedLimit)
  return commands
    .map((command) => {
...
async function defaultHandler(command, context = {}) {
  const args = Array.isArray(context.args) && context.args.length ? `\nArguments: ${context.args.join(' ')}` : ''
  return [
    `Dēmonyx specialist prepared: ${command.name}`,
    `Category: ${command.category}`,
    `Capability: ${command.capability}`,
    `No external state was changed.${args}`,
    'Bind an authorized handler to enable side effects for this capability.',
  ].join('\n')
}
