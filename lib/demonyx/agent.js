...
async function run(input, context = {}) {
  ...
  if (normalized === 'status' || normalized === 'health' || normalized === 'ping') {
    return [`Dēmonyx specialist online`, `Commands: ${registry.size}`, `Node: ${process.version}`, `Platform: ${process.platform}`, `Memory: ${Math.round(os.freemem() / 1024 / 1024)} MB free`].join('\n')
  }
