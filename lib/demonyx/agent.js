...
const { LocalMoE } = require('./moe')

bindBuiltins(registry)
const moe = new LocalMoE()
...
function help(query) {
  ...
  return [title, `Registered commands: ${registry.size}`, 'Local MoE: `.dx moe <request>`', ...matches.map(compact)].join('\n')
}
...
async function run(input, context = {}) {
  ...
  if (normalized === 'moe' || normalized === 'route' || normalized === 'local') return moe.respond(query, { sessionId: context.logicalSessionId || context.jid || context.sender })
  if (normalized === 'status' || normalized === 'health' || normalized === 'ping') {
    const summary = telemetry.summary()
    const model = moe.model
    return [`Dēmonyx specialist online`, `Commands: ${registry.size}`, `MoE experts: ${Object.keys(model.experts || {}).length}`, `Requests recorded: ${summary.total}`, `Node: ${process.version}`, `Platform: ${process.platform}`, `Memory: ${Math.round(os.freemem() / 1024 / 1024)} MB free`].join('\n')
  }
  if (normalized === 'about' || normalized === 'info') {
    return 'Dēmonyx is an additive specialist layer for the Levanter-compatible WhatsApp bot. It keeps the existing plugin system and adds a declarative, searchable, extensible command registry plus a local-first mixture-of-experts router.'
  }
...
module.exports = { run, help, categories, moe }
